...
Matrix standardMultiplySeq(const Matrix& A, const Matrix& B) {
    int n = A.size();
...
Matrix addSeq(const Matrix& A, const Matrix& B) {
    int n = A.size();
    Matrix C(n, vector<int>(n, 0));
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++)
...
Matrix subtractSeq(const Matrix& A, const Matrix& B) {
    int n = A.size();
    Matrix C(n, vector<int>(n, 0));
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++)
...
Matrix strassenSequential(const Matrix& A, const Matrix& B) {
    int n = A.size();
    if (n <= STRASSEN_THRESHOLD) return standardMultiplySeq(A, B);
    ...
    Matrix M1 = strassenSequential(addSeq(A11, A22), addSeq(B11, B22));
    Matrix M2 = strassenSequential(addSeq(A21, A22), B11);
    Matrix M3 = strassenSequential(A11, subtractSeq(B12, B22));
    Matrix M4 = strassenSequential(A22, subtractSeq(B21, B11));
    Matrix M5 = strassenSequential(addSeq(A11, A12), B22);
    Matrix M6 = strassenSequential(subtractSeq(A21, A11), addSeq(B11, B12));
    Matrix M7 = strassenSequential(subtractSeq(A12, A22), addSeq(B21, B22));
...
// Параллельный алгоритм Штрассена
...
#pragma omp parallel sections
    {
#pragma omp section
        M1 = strassenParallel(add(A, A), add(B, B));

#pragma omp section
        M2 = strassenParallel(add(A, A), B);

#pragma omp section
        M3 = strassenParallel(A, subtract(B, B));

#pragma omp section
        M4 = strassenParallel(A, subtract(B, B));

#pragma omp section
        M5 = strassenParallel(add(A, A), B);

#pragma omp section
        M6 = strassenParallel(subtract(A, A), add(B, B));

#pragma omp section
        M7 = strassenParallel(subtract(A, A), add(B, B));
    }
